...
names(df) <- c('category', 'name', 'email', 'phone', 'extra','extra_2', 'semester', 'year')
...
df <- df %>% fill(c(category, semester, year), .direction ='down')

#Cleaning up Category
df <- df %>% mutate(category = str_to_title(str_replace_all(category, '_', ' ')))

#Separate out 2019 & 2020 data to combine two name columns into one
df_19 <- df %>% filter(year >= 2019) %>% select (-extra_2)
df_rest <- df %>% filter(year < 2019) %>% select(-c(extra, extra_2))
...
write.csv(df, "../All WEC Special Volunteers 2006 to Fall 2020.csv", row.names=FALSE)
